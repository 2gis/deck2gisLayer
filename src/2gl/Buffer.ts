...
export class Buffer {
  ...
  constructor(
    ...
    this.byteLength =
      typeof initData === "number" ? initData : initData.byteLength;
    ...
    if (
      isElementArray &&
      !supportedElementArrayTypes.includes(this.options.dataType)
    ) {
      console.warn(
  ...
  private bindVertexAttribDivisor(
    ...
    if (options.instanceDivisor && this._hasRealWebGLContext()) {
      if (gl instanceof WebGLRenderingContext) {
        if (instancesExt) {
          instancesExt.vertexAttribDivisorANGLE(
            location,
            options.instanceDivisor,
          );
        } else {
          console.error(
