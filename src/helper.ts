...
import { createFramebufferMSAA, createProgram, createRenderTarget, createVao, wrapFramebuffer } from './renderer';
...
export function initDeck(map: Map, Deck: any, deckProps?: DeckRenderProps): Deck {
    ...
    const deckAntiaiasingMode = (deck.props._antialiasing as AntiAliasingMode) || 'none';
...
export function onMapResize(
    ...
    if (
        ...
        (!msaaFrameBuffer || (msaaFrameBuffer.width === targetTextureWidth && msaaFrameBuffer.height === targetTextureHeight))
    ) {
        return;
    }
    ...
    if (msaaFrameBuffer) {
        ...
        const colorRb = gl.getFramebufferAttachmentParameter(gl.FRAMEBUFFER, gl.COLOR_ATTACHMENT0, gl.FRAMEBUFFER_ATTACHMENT_OBJECT_NAME) as WebGLRenderbuffer | null;
        const depthRb = gl.getFramebufferAttachmentParameter(gl.FRAMEBUFFER, gl.DEPTH_ATTACHMENT, gl.FRAMEBUFFER_ATTACHMENT_OBJECT_NAME) as WebGLRenderbuffer | null;
        gl.bindFramebuffer(gl.FRAMEBUFFER, null);
        if (colorRb) {
    ...
    (deck.props as CustomRenderInternalProps)._2gisData._2gisCurrentViewport = undefined;
}
 ...
 * @internal
 */
export function drawLayer(deck: Deck, map: Map, layer: Deck2gisLayer<any>, target?: any): boolean {
...
function isIncludeLayer(deck: Deck, layer: Deck2gisLayer<any>): boolean {
    ...
    if (!(deck as any).layerManager.layers.some((deckLayer: any) => layer.id === deckLayer.id)) {
        return false;
    }
    return true;
}
